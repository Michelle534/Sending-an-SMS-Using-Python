...
def Book():
    ...
    if '' in Data:
        ...
        while True:
            ...
            phno = phone[i]
            text = "Dr."+str(DoctorName)+", you have an appointment with "+StudentName+" of class: "+str(StudentGrade)+StudentClass+" roll number: "+str(StudentRoll)+" slot: "+str(Slot)
            payload = f"sender_id=FSTSMS&message={text}&language=english&route=p&numbers={phno}"#connecting to website
            headers = {
            'authorization': "FoLEYbSTcWHiCaIXzPqvmUk1Vw0f6DdK7lsAy8eGRJrjnpgxMNrPtRKz0WVux3nw4HkChbpXD7FGgAQc", #special API key to send message via fast sms
